...
sleep 2;
echo ''
/bin/bash /tmp/apache-custom/configure --prefix=/usr/local/apache2-8082/ --enable-ssl=static --with-mysql --with-ssl=/usr/lib64/openssl/
sleep 2;
make && make install
sleep 5;
echo ''
...
sleep 2;
rm -Rf /usr/local/apache2-8082/conf/httpd.conf
rm -Rf /usr/local/apache2-8082/conf/extra/http-mpm.conf
cp /tmp/apache-custom/file/libphp5.so /usr/local/apache2-8082/modules/
...
wget -q -O /usr/local/apache2-8082/conf/httpd.conf https://raw.githubusercontent.com/fajarhide/apache-custom/master/conf/httpd.conf
wget -q -O /usr/local/apache2-8082/conf/extra/http-mpm.conf https://raw.githubusercontent.com/fajarhide/apache-custom/master/conf/extra/http-mpm.conf
...
/usr/local/apache2-8082/bin/apachectl -k start
sleep 5;
clear
echo ''
echo '... Great Jobs..!'
...
echo '... ------------------------------- '
echo '... Start: /usr/local/apache2-8082/bin/apachectl -k start '
echo '... Stop: /usr/local/apache2-8082/bin/apachectl -k stop '
echo '... Graceful: /usr/local/apache2-8082/bin/apachectl -k graceful '
echo '... Graceful: /usr/local/apache2-8082/bin/apachectl -k restart '
echo ''
rm -Rf install-8082.sh
rm -Rf /tmp/apache-custom